    ...
    async def _write_direct_with_refresh(
        ...
        *,
        ...
        try:
            ...
            if context_type == "signal":
                # Signal writes (e.g. OpenWebUI feedback) intentionally bypass
                # semantic refresh — raw user signals must not be routed into
                # memory vectorization. Release the lock immediately and report
                # result_wait=True since there is no async work to wait for.
                queue_status = None
                await lock_manager.release(handle)
                lock_released = True
                result_wait = True
